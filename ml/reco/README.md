...
```
후보 미션 → MissionFeatures 신호 6개 ─┬─→ 규칙 점수 (설명 칩·콜드스타트)
                                     └─→ 학습된 완료 확률 (reranker.json)
                        최종 = (1−λ)·규칙정규화 + λ·학습확률  → 다양성 감점 → 상위 3
```

- 신호: `explicit_pref, implicit_affinity, difficulty_fit, proximity, popularity, time_of_day_fit` (0~1).
  `features.py` = 앱의 `domain/MissionFeatures.kt` 와 **정확히 같아야 한다**(시간대 표·감쇠 상수 포함).
- 모델: 가중치 6 + 절편 1 (`app/src/main/assets/reranker.json`). 없으면 앱은 규칙 기반으로 폴백.
...
../.venv/bin/python train_reranker.py --rows rows.csv \
    --out ../../app/src/main/assets/reranker.json --blend 0.6 --version reranker-lr-sim-3
...
## 현재 모델 (`reranker-lr-sim-3`)

실제 로그가 없어 **시뮬레이터로 학습**했다. 시뮬레이터의 참 선호는 규칙 고정 가중치와 다르게
설정돼 있고(인기도·거리를 규칙은 크게 잡지만 실제로는 거의 무의미, 난이도·시간대 적합도는 규칙보다 훨씬 중요),
사용자마다 앱을 여는 시각을 아침·점심·저녁에 몰리게 두고, 완료율은 현실적으로 낮게(31%) 둬서
"상위 3건" 정렬이 실제로 변별되도록 했다.
...
학습된 가중치가 규칙의 손튜닝 오류(proximity 1.5→0.21, popularity 1.0→−0.06,
difficulty 1.0→3.16, time_of_day 1.0→2.46)를 교정한다.
`user_missions` 로그가 쌓이면 같은 파이프라인으로 재학습해 `--from-firestore` 로 교체한다
(완료 시각에서 KST 기준 `time_of_day_fit` 복원).
